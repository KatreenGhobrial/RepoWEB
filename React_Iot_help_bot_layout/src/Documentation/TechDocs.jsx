...
export default function TechDocs() {
  ...
  if (loading) {
    return <div className="p-12 text-center text-slate-500 dark:text-slate-400">Loading documents...</div>;
  }
    ...
    return (
      <>
        ...
        <div className="mb-6 flex items-center gap-3">
          <button onClick={backToList} className="text-sm font-semibold text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-white transition-colors">
            ← Back to list
          </button>
          ...
          {isEditing && (
            <>
              <button onClick={handleSave} className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-bold bg-slate-950 text-white hover:bg-slate-800 dark:bg-white dark:text-zinc-900 dark:hover:bg-slate-200 transition-colors">
                <LuSave /> Save
              </button>
        ...
        <div className="bg-white dark:bg-zinc-800 rounded-3xl border border-slate-200 dark:border-zinc-700 shadow-sm p-7">
          {isEditing ? (
            <div className="space-y-5">
              ...
              <LabeledInput label="Content (Markdown)">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                  ...
                  />
                  <div className="border border-slate-200 dark:border-zinc-600 rounded-2xl px-5 py-4 bg-slate-50 dark:bg-zinc-900 min-h-[400px] overflow-auto prose prose-sm dark:prose-invert max-w-none">
                    <p className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wide mb-3">Preview</p>
            ...
            <div>
              <div className="flex flex-wrap items-center gap-3 mb-4">
                <h3 className="text-2xl font-bold text-slate-950 dark:text-white">{selectedDoc.title}</h3>
  ...
  return (
    <>
      ...
      {/* Filter + New Doc */}
      <section className="bg-white dark:bg-zinc-800 rounded-3xl border border-slate-200 dark:border-zinc-700 shadow-sm p-6 mb-8">
        <div className="flex flex-col sm:flex-row sm:items-center gap-4">
          <div className="flex items-center gap-2 flex-wrap">
            <LuFilter className="text-slate-500 dark:text-slate-400" />
            {filterButtons.map(b => (
              <button
                key={b.key}
                ...
                className={`px-3 py-1.5 rounded-xl text-sm font-semibold transition-colors ${
                  filterType === b.key
                    ? 'bg-slate-950 text-white dark:bg-white dark:text-zinc-900'
          ...
          <div className="sm:ml-auto">
            <button
              onClick={openCreate}
              className="flex items-center gap-2 bg-slate-950 text-white px-5 py-2.5 rounded-2xl font-bold hover:bg-slate-800 dark:bg-white dark:text-zinc-900 dark:hover:bg-slate-200 transition-colors"
            >
      ...
      {/* Document Cards */}
      {filtered.length === 0 ? (
        <div className="bg-white dark:bg-zinc-800 rounded-3xl border border-slate-200 dark:border-zinc-700 shadow-sm p-12 text-center">
          ...
          {filtered.map(doc => {
            ...
            return (
              <div
                key={doc._id}
                onClick={() => openDoc(doc)}
                className="bg-white dark:bg-zinc-800 rounded-3xl border border-slate-200 dark:border-zinc-700 shadow-sm p-6 hover:shadow-md hover:border-slate-300 dark:hover:border-zinc-500 cursor-pointer transition-all"
              >
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400 line-clamp-2 mb-3">{preview || 'No content'}</p>
                {doc.tags?.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 mb-3">
                    {(Array.isArray(doc.tags) ? doc.tags : []).slice(0, 4).map(tag => (
                      <span key={tag} className="text-xs bg-slate-100 dark:bg-zinc-700 text-slate-500 dark:text-slate-400 px-2 py-0.5 rounded">
                        #{tag}
                      </span>
                    ))}
                  </div>
