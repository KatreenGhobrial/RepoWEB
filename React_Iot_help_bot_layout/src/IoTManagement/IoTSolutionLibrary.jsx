    ...
    <>
      ...
      <div className="mb-6">
        <input
          ...
          className="w-full max-w-lg border border-slate-300 rounded-2xl px-5 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500 bg-white dark:bg-zinc-900 shadow-sm"
        />
      </div>
      ...
      {/* Count */}
      <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">{results.length} item{results.length !== 1 ? 's' : ''} found</p>

      {/* Hardware Tab */}
      {activeTab === 'hardware' && (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {results.map(hw => (
            <div key={hw.name} className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl p-5 hover:shadow-md transition-shadow">
      ...
      {activeTab === 'protocols' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {results.map(proto => (
            <div key={proto.name} className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl p-5 hover:shadow-md transition-shadow">
      ...
      {activeTab === 'cloud' && (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {results.map(cp => (
            <div key={cp.name} className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl p-5 hover:shadow-md transition-shadow">
              <div className="flex justify-between items-start mb-3">
                <div className="flex items-center gap-2">
      ...
      {activeTab === 'software' && (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {results.map(sw => (
            <div key={sw.name} className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl p-5 hover:shadow-md transition-shadow">
              <div className="flex justify-between items-start mb-3">
                <div className="flex items-center gap-2">
      ...
      {results.length === 0 && (
        <div className="text-center py-16">
          ...
          <p className="text-slate-500 dark:text-slate-400 font-medium">No results found for "{search}"</p>
